...
5. The active agent is the holder: it observes workers, heartbeats by claim ID/token/current revision, reconciles decisions, checks evidence, and writes in source/item order through the selected guard. Persist each specification plus `refined:` checkpoint and refresh `refinement: complete`. Release item claims after each barrier; retain a source claim through all selected waves and release only after the final barrier or durable stop.
...
- A whole Backlog.md project directory is a writable collection source. Discover through supported `backlog` CLI/MCP reads. Outside `mode:spec-only`, claim each item locally, project `@<assignee>` through a guarded task edit, and run every provider mutation through `backlog-claim exec ... -- backlog ...`; never edit task files directly. In `mode:spec-only`, use only specification-bearing provider operations and leave workflow state unchanged.
- Every remote item is provider-owned and remote-only for refinement: never create a local Markdown mirror. GitHub normal-mode writes run through guarded `gh`; Linear normal-mode refinement stops before work unless its configured first-party integration supplies a complete native claim transaction.
...
Remote references authorize updating only exact resolved item content and, outside `mode:spec-only`, its matching `refined:` checkpoint, bounded claim lifecycle, and human-required blocked transition through the selected fenced adapter. Provider state remains authoritative; the same-host lease is coordination only, while snapshots and handoffs are read-only context. No other workflow mutation, item creation/deletion, or out-of-scope write is authorized.
...
2. Before fetching/pinning specification content, snapshots, delegation, or editing, preflight the exact content update. Outside `mode:spec-only`, also preflight `backlog-claim` key/acquire/heartbeat/release and guarded writes for Backlog.md/GitHub, or the complete provider-native claim transaction required for Linear, plus the durable `refined:` checkpoint and possible human-blocker operations. Preflight is non-mutating.
3. If any current-mode operation or fence is unavailable, unsupported, unauthenticated, cross-host unsafe, or scope-invalid, stop that item's flow before snapshot/delegation/edit, report the exact limitation, and do not describe it as refined, claimed, or durably blocked. Never downgrade to an assignee/comment/status claim. Continue only with another independent item in the same dependency-ready wave whose own preflights succeeded.
...
8. Outside `mode:spec-only`, heartbeat/revalidate, write specification plus `refined:` checkpoint through the current claim ID/token/revision guard, and refresh `refinement: complete`; release item claims per barrier and a source claim after all selected waves. In `mode:spec-only`, write only specification content and verify its provider version. Missing completion, stale ownership, file/provider conflict, failed write, or failed release stops dependents.
...
If a remote item changes later, refresh it before refining again and use a fresh read-only snapshot when needed. Loose Markdown and Backlog.md projects remain first-class inputs: preserve Markdown style and, outside `mode:spec-only`, use the selected guarded operation rather than direct source/task-file edits.
