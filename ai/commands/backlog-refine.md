...
- Because claims are provider state, this mode does not acquire them. Use one mutating session, do not dispatch item-level refinement workers, and retain only optional read-only evidence gathering. If safe completion requires cross-session exclusion, report that `mode:spec-only` cannot provide it rather than writing a claim.
...
3. Narrow the wave to items without shared implementation coordination surfaces. With `item-claim`, generate unique claim/worker-attempt IDs and acquire each `refine:<item-id>:<provider-version>` using graph/version eligibility. With fenced `source-claim`, acquire the source once and serialize all authoritative writes under that epoch while workers remain proposal-only. Revalidate dependencies after acquisition. Resolve ambiguous results by exact claim ID; never substitute a dependent.
...
5. The active agent is the actual holder: it observes workers, heartbeats by claim ID/current revision, reconciles decisions, checks evidence, and writes in source/item order. Persist each specification plus `refined:` checkpoint and refresh `refinement: complete`. Release item claims after each barrier; retain a source claim through all selected waves and release only after the final barrier or durable stop. Do not enter the next wave after any unresolved failure.

One selected item stays in the active agent but follows the same claim-before-read/edit and fenced-write rules. A coherent stop after a subtask/specification checkpoint releases the claim while preserving resumable `in-progress` state. An unexpected stop is recovered only after lease expiry and atomic reclaim; never steal an active claim or use an ordinary status, assignee, comment, branch, or handoff as a lock.
...
Provider operations stay narrow: use first-party Linear tooling for Linear, `gh` for GitHub Issues, and supported Backlog.md operations. Remote writes are limited to exact resolved items, refined content, the matching normal-mode `refined:` checkpoint and claim lifecycle, and the human-required blocked transition. Do not make other workflow changes or touch items outside scope. In `mode:spec-only`, do not use claims, progress/checkpoint, status, review, comments, or dependency operations.
...
Remote references authorize updating only exact resolved item content and, outside `mode:spec-only`, its matching `refined:` checkpoint, bounded claim lifecycle, and human-required blocked transition through first-party tooling. Provider state remains authoritative; snapshots/handoffs are read-only context, never shadow state. No other workflow mutation, item creation/deletion, or out-of-scope write is authorized.

Before refining:

1. Resolve each remote collection or item reference with the provider-native integration: Linear MCP/tooling for Linear, `gh` for GitHub Issues, and the supported `backlog` CLI/MCP for Backlog.md projects. Establish exact selected identities and the complete dependency graph in source/selector order. If a required integration is unavailable or unauthenticated, stop and report that exact limitation.
2. Before fetching/pinning specification content, snapshots, delegation, or editing, preflight the exact content update. Outside `mode:spec-only`, also preflight the durable `refined:` checkpoint, atomic claim/heartbeat/release, claim-ID/revision fencing on actual writes, and possible human-blocker operations. Use only non-mutating capability/authorization/scope checks. In `mode:spec-only`, preflight specification content only.
3. If an operation required by the current mode is unavailable, unauthenticated, unsupported, or scope-invalid, stop that item's flow before snapshot, delegation, or edit, report the exact limitation, and do not describe it as refined, claimed, or durably blocked. Never downgrade to an ordinary comment/status claim. Continue only with another item already in the same dependency-ready wave whose own preflights succeeded.
...
5. Outside `mode:spec-only`, acquire current-wave item claims or one fenced source claim before snapshots/workers. On item conflict, refresh and remove that item plus gated dependents; on source conflict, schedule nothing in that source. In `mode:spec-only`, use the single-mutator rule.
...
8. Outside `mode:spec-only`, heartbeat/revalidate, write specification plus `refined:` checkpoint with claim ID/current revision, and refresh `refinement: complete`; release item claims per barrier and a source claim after all selected waves. In `mode:spec-only`, write only specification content and verify its version. Missing completion, ambiguous/stale ownership, failed writes, or failed release stop dependents.
...
- Outside `mode:spec-only`, verify item/source claim receipts, every fenced specification and `refined:` receipt, refreshed `refinement: complete`, and final releases. If all roots are blocked, done, or actively claimed, confirm no worker was scheduled and report them rather than selecting dependents.
