...
6. Commit only task-related changes with a concise message. Do not push, merge,
   or open a PR — this command hands off local work; integration is a follow-up
   command's job (`/backlog-implement` for worktree work, `/pr-babysit` for an
   open PR).
