...
Implement the selected backlog item or items from `$ARGUMENTS` through all refined tasks in isolated worktrees/subtrees. When only a backlog source is supplied, process its first open item. Preserve existing unstaged work, verify and durably record each coherent task, commit only task-related work, then verify and integrate each completed item per the repo's flow (merge to local main or open a PR).
...
5. Record every completed task in the item's writable backlog source. For remote-only items, post an `implemented:` progress comment after every task and update final workflow status through the authorized first-party flow. If the tool cannot write comments, do not begin the remote-only item because neither task checkpoints nor completion can be recorded durably.
...
2. Identify eligible items from `$ARGUMENTS`. Explicit item IDs, titles, or ranges select those items and must all be processed in supplied order. Naming only a backlog file or remote project selects its first open item. Never advance beyond that explicit or default scope.
...
One invocation completes every eligible selected item, in supplied order, through all of its refined coherent tasks. Treat each task as a bounded implementation and verification checkpoint: complete it, verify it, commit it, persist its durable state, then continue to the next task in the same item. After an item is complete, verified, and integrated, continue to the next explicitly selected item; a source-only invocation stops after its first open item. Stop earlier only for a genuine blocker or failed durable-state write-back that prevents safe continuation.

The writable backlog item is authoritative state; remote item comments are authoritative for remote-only items. The final report is only a reproducible summary. After every completed task, before beginning the next task, persist:

- item status: `in-progress`, `verification-pending`, `blocked`, or `complete`, following existing source vocabulary
...
`implemented: <task>; commit: <commit>; verified: <command/result>; status: <in-progress|verification-pending|complete>; next: <task|VERIFY|COMPLETE>; remaining: <acceptance criteria|none>`
...
2. Commit the coherent implementation. Record its task name, commit, verification result, current `in-progress` or `verification-pending` status, exact next task or `VERIFY`, and remaining acceptance criteria in the writable backlog source; when the implementation commit cannot be named in the same commit, immediately add one state-only commit naming it.
3. For remote-only items, post the equivalent `implemented:` comment. Do not use the final report as the progress store. If write-back fails, stop before the next task and leave `NEXT CONTEXT REQUIRED`.
4. When another task remains, re-read the durable item state, select that exact next refined task, and continue within this invocation. Never cross into another item until the current item is complete, verified, and integrated.
5. When no task remains, persist `verification-pending; next: VERIFY; remaining: none`, verify that every acceptance criterion is covered, then run the final verifier before recording `status: complete`.
...
- At the end, make the continuation status unambiguous:
  - `NEXT CONTEXT REQUIRED` when there is backlog work outside a source-only invocation's first-item scope, any explicitly selected item remains unfinished, any task remains because of a genuine blocker or durable-state write-back failure, an oracle-confirmed blocker or missing product decision remains, scoped verification cannot be fixed in-repo, or a writable local markdown backlog file remains unarchived when archiving is required. Include the exact next item and task.
  - `BACKLOG COMPLETE AND ARCHIVED` only when every explicitly selected item is complete and no open item remains in a source-only backlog; all completed items are verified, committed, and integrated (merged locally or PR opened); each writable local markdown backlog file has been archived; and each remote-only item has durable `status: complete; remaining: none` item state and is marked done or merged when the first-party tool supports that workflow transition. Include the exact final item completed and where it was archived, or the remote state and workflow transition applied.
...
- Before marking each item complete, run one final verifier agent over that item, with its acceptance criteria and accumulated implementation commits — not your conclusions — and treat any FAIL or UNVERIFIED criterion as open work.
...
- Integrate each item when it becomes complete, then continue to the next explicitly selected item. If the current item remains incomplete because of a genuine blocker or durable-state write-back failure, stop the invocation in supplied order, retain its isolated branch/worktree, and report its exact location; do not skip ahead to a later selected item.
- Start the final report with exactly one status line: `NEXT CONTEXT REQUIRED` or `BACKLOG COMPLETE AND ARCHIVED`.
- Then reproduce the durable state: backlog source, current item, completed tasks, implementation and state commits, verification, exact next item/task, retained worktree or integration result, archive location if applicable, ignored unrelated failures or dirty changes, blockers, and any failed remote write-back.
