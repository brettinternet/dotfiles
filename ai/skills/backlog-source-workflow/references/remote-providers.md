...
- Map Linear issue IDs, title/description, workflow state, priority/order, dependencies/relations, refinement/review checkpoints, and current item/source claim into `ItemState`. Keep UUID, URL, provider time, timestamps, and version/concurrency fields in `providerData`.
...
- Declare `item-claim` or fenced `source-claim` only when a first-party conditional transaction supplies eligibility revalidation, provider time, unique claim/revision, idempotent recovery, renew/release, and fencing on actual work mutations. Partial support is `none` and returns a capability error; labels, assignees, comments, and workflow status are not claims.
...
- Enumerate the complete collection with an explicit non-default bound such as `gh issue list --state all --limit 1000` and supported pagination until no next page remains. Fetch issues with `gh issue view` as needed for full dependency, refinement/review checkpoint, provider version/time, and repository-defined item/source claim metadata. An exhausted unknown bound is a diagnostic, not complete discovery.
...
- Record required `refined:`, `implemented:`, `reviewed:`, or `blocked:` progress through the authorized repository convention. A normal refinement's `refined:` checkpoint names its specification receipt/version; `mode:spec-only` leaves progress unchanged. If `gh` or the write fails, do not claim completion.
- Declare `item-claim` or fenced `source-claim` only when an explicitly documented repository/provider operation callable through `gh` provides eligibility revalidation, provider time, unique claim/revision, idempotent recovery, renew/release, and fencing on actual mutations. Otherwise capability is `none`. Comments, labels, assignees, Projects status, branches, and read-then-edit sequences are not claims.
