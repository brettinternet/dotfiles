...
**Discovery:** Resolve exact team/project/issue identities; ambiguous unqualified identifiers require clarification. Accept the contract's canonical `linear:<locator>` form and its `Linear <locator>` alias, including an exact project shortcode or team key such as `TICK`. Resolve direct URL/ID/key and exact-name matches before broader search. When `TICK` exactly identifies a team key, select that team's issue set; a project returned only by partial/fuzzy name search is not a competing candidate. If multiple distinct Linear objects match the locator exactly, report only those exact candidates as an explicit-reference ambiguity. If the first-party integration cannot resolve a qualified source, report the explicit Linear reference or integration as unresolved; never reclassify it as selector-only or source-free, and never consult repository metadata to choose among qualified-source candidates. Repository detection requires one configured Linear source and runs only for source-free input. Enumerate every page in a source-only collection. Map IDs, description, workflow state, priority/order, relations, and checkpoints into `ItemState`; retain UUID, URL, timestamps, and concurrency fields. Pass explicit review-group selectors unchanged and require exact members plus supported durable markers.

**Claims and mutation:** Use native fencing only when Linear provides conditional acquisition, authority time, claim revision, renewal, recovery, and fencing on actual writes. Otherwise derive the canonical Worklease item resource as `local-coordination` and follow [`claims.md`](claims.md). After acquisition, assign an unassigned issue to the authenticated user; preserve a different assignee and stop. Use first-party update/comment/state operations, preserving unpatched fields and retaining remote version receipts. Persist caller-authorized progress markers through Linear; `mode:spec-only` leaves progress/workflow state unchanged. Never run Linear calls through `worklease exec` or call assignment a claim.
...
**Discovery:** Resolve an exact repository/issue locator; a bare number needs a repository. Repository detection requires one unambiguous origin/configured repository. Enumerate the complete collection with an explicit high limit and pagination (for example `gh issue list --state all --limit 1000`), diagnosing exhausted unknown bounds. Use `gh issue view` for full state. Derive and merge the canonical Worklease item claim. Extract dependencies only from native relations exposed through `gh` or documented issue-body vocabulary; keep malformed declared references unresolved instead of inferring from arbitrary prose, labels, titles, or incidental `#` references. Resolve explicit review groups only through a documented repository/provider convention with exact members and durable markers.

**Mutation:** Refresh the issue and exact Worklease claim, then run each `gh` mutation through `worklease exec` only as a guarded local operation; retain the provider receipt and report the durable GitHub write as `LOCAL COORDINATION (UNFENCED)` unless GitHub itself atomically enforces a supplied provider version and returns evidence. `gh api --paginate` is allowed for reads. Persist caller-authorized progress through the repository convention; `mode:spec-only` leaves progress/workflow state unchanged. Failed/ambiguous writes remain incomplete until reconciled or expiry. An unavailable Worklease CLI, `gh`, or authorized operation is capability `none`; never bypass `gh`. Never treat comments, labels, assignees, Projects status, branches, or issue locks as claim state.
