...
- Map statuses, review markers, priorities, and dependency references from the existing vocabulary. An absent dependency target is unresolved, not complete. If the convention has no explicit ID, use its established file/heading identity and keep it stable across edits; do not manufacture a new format.
...
- `writeState` and `recordProgress` edit the existing item in its established style. Write the actual status/checklist/progress marker before reporting the checkpoint; a command handoff is not durable state.
...
- Map provider task IDs, title/body, status, priority, order, dependencies, and review metadata into `ItemState`. Refresh dependency targets with the same provider operation when the listing is incomplete.
- Use the provider's status vocabulary to determine terminal dependencies and review-pending work. Preserve Backlog.md's ordering and opaque fields in `providerData`.
...
- `recordProgress` uses the provider's task fields/comments/markers as exposed by CLI/MCP, not a local note. Keep remote-looking or command-local state out of the project unless the provider operation explicitly stores it.
