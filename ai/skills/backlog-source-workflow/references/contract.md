...
3. `selectNext(scope, mode)` evaluates terminal state, dependency readiness, blockers, and claims and returns one ordered claimable item or a structured `complete`, `blocked`, or `active-claims` result. The `blocked` result is reserved for provider-authored blockers, missing/cyclic dependency diagnostics, or evidence-backed external/human decisions; an unfinished dependent of a defined backlog item remains dependency-gated (`ready after <item>`). It never returns a review group.
...
- An ordinary incomplete prerequisite represented by another defined backlog item is a dependency gate, not a durable `BLOCKED` status. Keep its dependent visible, report `ready after <item>` when readiness is surfaced, and do not mutate the dependent to blocked merely because the prerequisite is unfinished or claimed.
...
- `BLOCKED` is reserved for provider-authored blockers, missing dependency references, cycle members, or evidence-backed external/human decisions. Ordinary dependency-gated dependents remain ineligible for the current wave without becoming blocked.
...
When no item is claimable, return the reason rather than reaching past the gate: `complete` when all scoped work is terminal, `blocked` only when unfinished work has no satisfiable ready root because roots are provider-blocked, have missing/cyclic references, or carry an evidence-backed external/human blocker, or `active-claims` with the active claim owners/expiry and dependency chains they hold when all otherwise-progressable roots are leased. Ordinary dependents waiting on defined unfinished backlog items remain dependency-gated (`ready after <item>`), not `blocked`; never select a completed item, a dependent of unfinished work, or a later item merely to avoid returning no work.
