...
Before drafting findings, read the complete existing PR discussion and review
...
Compare each potential finding with that discussion. Do not include a duplicate
...
- At most 1-4 priority comments, only for issues that could plausibly break behavior, data, deployments, security, or compatibility. Do not include issues already discussed in the PR.
...
- Attach every finding that maps to a changed line as a real file-line comment
  ...
  changed line fits.
- Approve the PR with a short review comment when there are no material concerns.
- When there are material concerns, submit a `COMMENT` review; never submit
  `REQUEST_CHANGES`, even for a material concern.
...
can answer or push back on. Do not stack questions; for a material concern, ask
one direct question that names it.
