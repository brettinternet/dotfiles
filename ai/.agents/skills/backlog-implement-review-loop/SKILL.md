---
name: backlog-implement-review-loop
description: Run one bounded implementation/review pass from a backlog
---
<!-- Generated by install-agent-commands. -->

When explicitly invoked as `$backlog-implement-review-loop`, treat the text supplied with the invocation as `$ARGUMENTS`. Where the command uses `$1`, `$2`, or `$@`, interpret them as the first, second, or all supplied arguments.
