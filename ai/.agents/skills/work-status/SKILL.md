...
When explicitly invoked as `$work-status`, treat the text supplied with the invocation as `$ARGUMENTS`. Where the command uses `$1`, `$2`, or `$@`, interpret them as the first, second, or all supplied arguments.
