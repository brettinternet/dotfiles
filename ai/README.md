...
`worklease` is the shared same-host claim authority (pinned to `v0.2.0` in the mise tool configuration): derive canonical resources with `worklease key`, acquire fresh ownership epochs, heartbeat before half the TTL, guard local commands with `worklease exec`, replace loose Markdown with expected-hash `worklease replace-file`, and release only after a verified provider checkpoint. Worklease's `fenced` guarantee covers only its named local operation; Backlog.md, GitHub, Linear, and other provider CLI/API writes remain `LOCAL COORDINATION (UNFENCED)` unless the provider itself supplies conditional-write evidence. Backlog.md provider writes and Git integration additionally acquire a short `local:provider-transaction:<canonical-common-dir>` coordination claim and use `worklease exec --git-primary`; this claim is separate from the long item pass and is never held during implementation or review. The `worklease-workflow` skill is vendored under `ai/skills/worklease-workflow`; `backlog-source-workflow` maps its provider-specific scheduling and receipts onto that contract. Unsafe or ambiguous canonical checkout resolution returns `WAIT` instead of falling back to feature-worktree provider state. Coherent handoffs checkpoint and release immediately; abandoned leases expire. Dependency waves remain authoritative, so another local session may take an independent ready root but never a dependent of unfinished or locally claimed work.
