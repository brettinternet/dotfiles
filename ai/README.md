...
Every iteration starts fresh and reconstructs provider state plus active claims instead of relying on chat history. A Backlog.md path supplied from any Git worktree resolves to the repository's unambiguous primary/control checkout; all provider reads and writes happen there, while implementation worktrees contain code only. The pass order is claim, canonical `in-progress` checkpoint, then isolation. Each coherent pass checkpoints canonical progress before releasing its one-pass lease.

`backlog-claim` coordinates cooperating agents on one host across repository worktrees: Backlog.md and GitHub use fenced item leases around CLI mutations, while loose Markdown uses one fenced source lease plus expected-hash replacement. A separate short provider/repository mutation transaction lock serializes canonical provider writes, provider-state commits, and Git integration, but is never held during implementation or review. Independent ready items can therefore proceed concurrently in separate worktrees while shared control-plane mutations serialize. Providers without mutation fencing, including Linear, automatically use a canonical `local-coordination` lease so work can proceed; direct first-party writes require lease/provider pre/post checks and are reported as `LOCAL COORDINATION (UNFENCED)`. Backlog.md and Linear assign unassigned work to the active provider user for visibility where supported; assignment is not the claim. Neither coordination-only leases nor the mutation transaction lock fences provider writes or workers on another host. Unsafe or ambiguous canonical checkout resolution returns `WAIT` instead of falling back to feature-worktree provider state. Coherent handoffs checkpoint and release immediately; abandoned leases expire. Dependency waves remain authoritative, so another local session may take an independent ready root but never a dependent of unfinished or locally claimed work.
