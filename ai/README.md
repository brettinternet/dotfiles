...
Every iteration starts fresh and reconstructs provider state plus active claims instead of relying on chat history. `backlog-claim` serializes cooperating agents on one host across repository worktrees: Backlog.md and GitHub use item leases around CLI mutations, while loose Markdown uses one source lease plus expected-hash replacement. Backlog.md also assigns the task to `@<assignee>` for visibility; assignment is not the lock. Linear remains fail-closed unless its configured integration supplies native fenced claims, and cross-host workers require a shared CAS service. Coherent handoffs checkpoint and release immediately; abandoned leases expire. Dependency waves remain authoritative, so another session may take an independent ready root but never a dependent of unfinished or claimed work.
