...
- Destructive Git cleanup is pre-authorized only inside a verified non-primary HWT-managed or session-owned worktree. Agents may discard its changes, remove it even when dirty or unmerged, and delete or move its associated local branch. Verify the exact workspace, path, and ref first; require approval for primary or unknown checkouts and never remove the default branch, remote branches, or anything explicitly marked user-owned or `keep`.
