...
- Prefer `mise exec worktrunk -- wt switch --create <branch> --base <base> --no-cd --format=json` for command-created worktrees. Work from the returned path; repository `.config/wt.toml` hooks prepare it, and the personal post-start hook opens a Herdr workspace when running inside Herdr. Review and approve project hooks before unattended creation; do not bypass required setup.
