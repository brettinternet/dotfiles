...
- Default every agent-created worktree to `<repo>/.worktrees/<name>`. Use another location only when the user, repository configuration, or harness explicitly specifies it; harness-provided isolation counts as explicit.
...
- Destructive Git cleanup is pre-authorized only inside a verified non-primary HWT-managed or session-owned worktree. Agents may discard its changes, remove it even when dirty or unmerged, and delete or move its associated local branch. This authorization includes the required `--force`; do not request separate approval for verified managed cleanup.
- Before HWT cleanup, use `hwt list --cwd <repository-path>` as the authoritative source of the workspace ID, checkout path, and branch; never infer ownership from names. From a surviving workspace, remove the worktree and its branch in one Bash call (for example, `hwt remove --workspace <workspace-id> --force --json && git -C <repository-path> branch -D <branch>`) so ownership is verified before the worktree disappears.
...
- For exact-SHA detached checkouts or when HWT/Herdr is unavailable, use `git worktree` and the same `.worktrees` default. Plain Git worktrees are eligible for unattended destructive cleanup only from the session running inside that linked worktree; otherwise require approval or leave cleanup to the harness.
